...
const insertArea = (area) => 
  new Promise((resolve, reject) => {
    ...
    .then(({ insertedIds, result }) => {
      ...
      const spotIDsFormatted = insertedIds.map((v, i) => {console.log(v, i); return v;});
      process.exit(1);
      // const spotsIDs = { spots: insertedIds.map((v, i) => ) };
      const newArea = { ...area, ...spotsIDs };
      console.log(newArea);
      Area.collection.insertOne(newArea, (res) => {
        // console.log(`>> INSERTED {${area.name}} : [${insertedIds.length}] SPOTS`);
        resolve(insertedIds);
      });
    });
  });
